...
export const Route = createFileRoute("/_app/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — Apex Invest" }] }),
  component: Dashboard,
});

const data = [
  { d: "Mon", v: 1200 }, { d: "Tue", v: 1900 }, { d: "Wed", v: 1700 },
  { d: "Thu", v: 2400 }, { d: "Fri", v: 2200 }, { d: "Sat", v: 3100 }, { d: "Sun", v: 3600 },
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-3">
        ...
        <Badge className="gradient-gold text-gold-foreground border-0 px-3 py-1.5">Active Plan · Plan 5</Badge>
      </div>
          ...
          <Card key={s.label} className="glass border-border/40">
            <CardContent className="p-5">
              <div className="flex items-center justify-between mb-3">
                <div className={`size-10 rounded-xl bg-accent/40 grid place-items-center ${s.accent}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="glass border-border/40 lg:col-span-2">
          <CardHeader><CardTitle>Earnings Growth</CardTitle></CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                <Tooltip contentStyle={{ background: "oklch(0.23 0.05 280)", border: "1px solid oklch(0.32 0.05 280)", borderRadius: 12 }} />
                <Area type="monotone" dataKey="v" stroke="oklch(0.83 0.16 85)" strokeWidth={2.5} fill="url(#g)" />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <Card className="glass border-border/40">
        <CardHeader><CardTitle className="flex items-center gap-2"><Users className="size-5" /> Referral Network</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-3 gap-4">
          {[{ l: "Level 1", n: 12, t: "Direct" }, { l: "Level 2", n: 38, t: "Second step" }, { l: "Level 3", n: 96, t: "Third step" }].map((r) => (
      ...
      <Card className="glass border-border/40">
        ...
        <CardContent>
          <div className="flex justify-between text-sm mb-2"><span>Level 5 → Level 6</span><span className="text-gold font-semibold">63 / 100 pts</span></div>
          <Progress value={63} className="h-3" />
          <p className="text-xs text-muted-foreground mt-3">Earn 37 more points to unlock Level 6 and a ₨ 35,000 reward.</p>
        </CardContent>
      </Card>
    </div>
  );
